#!/usr/bin/env bash
# Build the AAudio probe for Android and run it on a connected device. The render
# (AAudioSink) path needs no permission and runs as a plain native binary from
# /data/local/tmp; the capture (AAudioSrc) path needs RECORD_AUDIO (an APK), so
# the probe attempts it best-effort and reports rather than failing.
#
# Prerequisites:
...
#
# Usage: tools/android-aaudio-smoke.sh [abi]
...
#    --platform 26: AAudio is API 26+.
echo ">> building android_aaudio_probe for $ABI ($TRIPLE)"
cargo ndk --platform 26 -t "$ABI" build --release -p g2g-plugins --features aaudio \
    --test android_aaudio_probe
...
BIN="$(find "target/$TRIPLE/release/deps" -maxdepth 1 -type f -executable \
    -name 'android_aaudio_probe-*' -printf '%T@ %p\n' 2>/dev/null \
...
DEV=/data/local/tmp/g2g_aaudio_probe
