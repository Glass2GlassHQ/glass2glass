#!/usr/bin/env bash
# Build the MediaCodec H.264 ENCODE probe for Android and run it on a connected
# device. The encoder uses ByteBuffers on both sides (NV12 in, Annex-B out), so
# the probe runs as a plain native binary from /data/local/tmp, no APK needed.
#
# Prerequisites:
...
#
# Usage: tools/android-mediacodec-enc-smoke.sh [abi]
...
echo ">> building android_mediacodec_enc_probe for $ABI ($TRIPLE)"
cargo ndk --platform 24 -t "$ABI" build --release -p g2g-plugins --features mediacodec \
    --test android_mediacodec_enc_probe
...
BIN="$(find "target/$TRIPLE/release/deps" -maxdepth 1 -type f -executable \
    -name 'android_mediacodec_enc_probe-*' -printf '%T@ %p\n' 2>/dev/null \
...
DEV=/data/local/tmp/g2g_mediacodec_enc_probe
