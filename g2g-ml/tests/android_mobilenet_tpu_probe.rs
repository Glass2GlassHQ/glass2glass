...
async fn classifier_runs_through_g2g_chain() {
    ...
    quantize.configure_pipeline(&tensor_caps(TensorDType::F32, vec![1, 3, SIZE, SIZE])).expect("configure quantize");
    ...
    let mut infer = OrtInference::from_memory_with_nnapi(&model)
        ...
        .with_tensor_input();
    infer.configure_pipeline(&tensor_caps(TensorDType::U8, vec![1, 3, SIZE, SIZE])).expect("configure inference");
    ...
    argmax.configure_pipeline(&tensor_caps(TensorDType::F32, vec![1, CLASSES as u32])).expect("configure argmax");
