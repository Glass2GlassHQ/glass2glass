...
fn tensor_caps(shape: Vec<u32>) -> Caps {
...
async fn yolo_detects_the_dog() {
    ...
    infer.configure_pipeline(&tensor_caps(vec![1, 3, SIZE, SIZE])).expect("configure inference");
    ...
    decode.configure_pipeline(&tensor_caps(vec![1, CHANNELS, ANCHORS])).expect("configure decode");
