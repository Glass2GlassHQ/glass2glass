    ...
    fn process<'a>(
        ...
        Box::pin(async move {
            match packet {
                ...
                // future PipelinePacket variants: no-op (per-input aggregator,
                // a blanket forward would duplicate the packet N times).
                _ => {}
            }
            Ok(())
        })
...
fn slot_byte_size(slot: &Caps) -> Option<usize> {
    ...
    let elem = match dtype {
        ...
        // A dtype added since: unknown element size, cannot size the slot.
        _ => return None,
    };
    dims.iter()
...
mod tests {
    ...
    fn slot(dims: &[u32]) -> Caps {
        Caps::Tensor {
            dtype: TensorDType::U8,
            shape: g2g_core::TensorShape::from_slice(dims).unwrap(),
    ...
    fn slot_byte_size_requires_batch_one_static_dims() {
        assert_eq!(slot_byte_size(&slot(&[1, 4])), Some(4));
        assert_eq!(slot_byte_size(&slot(&[1, 3, 2, 2])), Some(12));
        assert_eq!(slot_byte_size(&slot(&[2, 4])), None, "batch must be 1");
        assert_eq!(slot_byte_size(&slot(&[1, 0])), None, "dims must be static");
    }

    #[test]
    fn output_caps_stack_along_batch_dim() {
        let b = TensorBatcher::new(3, slot(&[1, 3, 2, 2])).unwrap();
        assert_eq!(
