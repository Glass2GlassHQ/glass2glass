...
/// Tensor element type as named in a safetensors header. The float types
/// ([`Dtype::F32`], [`Dtype::F16`], [`Dtype::Bf16`]) are convertible to `f32`
/// here (F16 / BF16 dequantized on the fly, since real checkpoints ship in half
/// precision and the inference elements are f32); the rest are recognised so a
/// mixed file parses and a non-float lookup fails loud instead of silently
/// misreading bytes.
...
pub enum SafeTensorsError {
    ...
    Missing,
    /// The tensor's dtype is not a float type (F32 / F16 / BF16), so `to_f32`
    /// cannot dequantize it.
    NotFloat,
    /// The tensor byte length is not a whole number of elements, or disagrees
...
impl TensorRef<'_> {
    ...
    /// Decode a float tensor to row-major `Vec<f32>`, dequantizing F16 / BF16 to
    /// f32 on the fly (real checkpoints ship in half precision; the inference
    /// elements are f32). Fails loud on a non-float dtype or a byte length that is
    /// not the shape's element count of the dtype's element size.
    pub fn to_f32(&self) -> Result<Vec<f32>, SafeTensorsError> {
        // Bytes per element for the float dtypes; non-float fails loud.
        ...
        Ok(match self.dtype {
            ...
            _ => unreachable!("non-float dtype rejected before decode"),
        })
    }
}

/// Dequantize one BF16 (bfloat16) value to f32. BF16 is simply the high 16 bits
/// of an IEEE-754 f32, so widening is a shift, no field surgery (NaN / Inf and
/// subnormals all fall out for free).
fn bf16_to_f32(x: u16) -> f32 {
    f32::from_bits((x as u32) << 16)
}

/// Dequantize one IEEE-754 half (F16) value to f32: rebuild the sign / exponent /
/// mantissa fields, normalizing subnormals and preserving Inf / NaN. Exact (every
/// f16 is representable in f32).
fn f16_to_f32(x: u16) -> f32 {
    ...
    let bits = if exp == 0 {
        ...
        // Inf (frac == 0) or NaN (frac != 0); shift the mantissa into place.
        (sign << 31) | 0x7f80_0000 | (frac << 13)
    } else {
        // Normal: rebias the exponent (15 -> 127) and widen the mantissa.
        let e = exp + (127 - 15);
...
mod tests {
    ...
    #[test]
    fn non_float_dtype_to_f32_fails() {
    ...
    fn bf16_dequantizes_to_f32() {
        ...
        assert_eq!(t.to_f32().unwrap(), vals.to_vec());
    }

    #[test]
    fn f16_dequantizes_normal_and_special_values() {
        // Hand-encoded IEEE half values: 1.0, -2.0, 0.5, +0, smallest subnormal
        // (2^-24), and +Inf. Normals / halves are exact in f32.
        let pairs: [(u16, f32); 6] = [
            (0x3C00, 1.0),
            (0xC000, -2.0),
            (0x3800, 0.5),
        ...
        let bytes: Vec<u8> = pairs.iter().flat_map(|(h, _)| h.to_le_bytes()).collect();
        let blob = blob_of("F16", &[pairs.len()], &bytes);
        let st = SafeTensors::parse(&blob).unwrap();
        let t = st.get("t").unwrap();
        assert_eq!(t.dtype, Dtype::F16);
        let got = t.to_f32().unwrap();
        for (i, ((_, want), g)) in pairs.iter().zip(&got).enumerate() {
            assert_eq!(g, want, "f16 value {i}");
        }
    }

    #[test]
    fn f16_len_mismatch_fails_loud() {
