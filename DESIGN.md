...
`ElementSlot` swaps what sits in a position. `GraphMutator` (M1115) changes the positions themselves: it splices a transform onto a live edge of a running `run_graph` / `run_graph_threaded` graph, and lifts one back off, while frames keep flowing. `run_graph_mutable` and `run_graph_threaded_mutable` hand the caller the handle beside the run future, the way `run_source_router_dynamic` does.
...
**Remove drains.** Parking the producer and dropping the link it gave up closes the removed element's input, so its arm consumes what is queued, forwards the results and ends; its output link is left on its own endpoint as it goes. Only then does the producer take that link over, so every frame that was queued at the removed element passes *through* it before the first bypassed frame arrives. The element itself is handed back to the caller. A caps-changing element's consumer is about to start receiving the producer's caps instead, so it must accept them (same feasibility test, refused otherwise) and is told with a `CapsChanged` ahead of the first frame. A removed element is not flushed: an element buffering frames internally loses them, so the operation suits stateless positions (an overlay, a metering probe) rather than a reordering one.

**Scope.** A transform position on a 1:1 edge, between a source or transform and a transform or sink. Tee, demux and muxer positions, and the source and sink ends, are refused with `MutationError::NotMutable`: a structural node's caps and pad model are not a splice point. Every operation completes at the producer's next packet boundary, so a producer that has gone quiet defers it rather than failing.
...
**Cost.** One relaxed load per push on a mutable run, and nothing at all on an ordinary one (the endpoint is `None`, the zero-alloc proofs of M1000 / M1009 are unaffected). A mutable run does drive each transform through its erased handle so a remove can give the element back, which costs that element the unboxed `process` future an ordinary run gives it, the same cost a borrowing graph pays.
