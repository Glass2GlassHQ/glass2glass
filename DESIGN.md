...
- **playbin (multi-stream front door)** (`std`, M376-M382). Beyond the M196
  ...
  only at runtime retype. The gst-launch front door is `playbin uri=X` (M382):
...
The **zero-copy DMABUF import** path exists at the ingest side: `appsrc` accepts a `MemoryDomain::DmaBuf` frame (`AppSrcFeed::push_dmabuf`), `BridgeGraph::push_dmabuf` feeds it, and the C-ABI `g2g_bridge_push_dmabuf` `dup`s a GStreamer buffer's dma-buf fd (GStreamer keeps the original; g2g's `OwnedDmaBuf` closes only the dup) so no pixel bytes are copied at the boundary. The dma-buf-**consuming** element now exists: `dmabuftowgpu` (`g2g-plugins`, the `dmabuf-wgpu` feature) imports a `MemoryDomain::DmaBuf` frame into a GPU-resident `wgpu::Buffer` via `VK_EXT_external_memory_dma_buf` (Vulkan `from_raw_managed` -> `create_buffer_from_hal`), so a bridge fragment like `dmabuftowgpu ! <wgpu compute>` runs the imported buffer on the GPU with no CPU copy. Validated on an RTX 3060 by exporting GPU memory as a dma-buf fd and re-importing it (a discrete GPU binds a GPU-visible dma-buf; a CPU/vmalloc-backed one, e.g. a USB webcam or udmabuf, it cannot, and the element returns `UnsupportedDomain` rather than a wrong result). What remains for a *fully automatic* GStreamer round-trip: the shell's `transform` auto-detecting dma-buf input and routing to `g2g_bridge_push_dmabuf` (it takes the mapped-bytes path by default), and an output side that **wraps** a produced GPU/`DmaBuf` frame back into a `GstBuffer` (a dma-buf allocator export). Those two wiring steps are the remaining GPU-track work; the import primitive and the consuming element are in place.
