...
| **M12** | Live-source surface: latency aggregation query (done — `LatencyReport`, folded into `RunStats::latency`); allocation query (done — `AllocationParams` / `MemoryDomainKind`, resolved into `RunStats::allocation`); live clock distribution (pending). |
