...
- **Validation:** an on-hardware round-trip on the RTX 3060 synthesizes a CUDA-resident NV12 surface (CUDA driver alloc + upload), encodes through `NvEnc`, and decodes the Annex-B back through `FfmpegVideoDec` to the original geometry; it skips cleanly with no NVIDIA GPU. The `nvenc` feature is CI-excluded (no NVENC runtime in CI). Deferred: system-memory NV12 input, HEVC, finite-GOP periodic IDRs (`repeatSPSPPS`), an output-bitstream-buffer pool (one per frame today), and runtime bitrate retarget (`nvEncReconfigureEncoder`). A matching native `NvDec` is the other half of a gst-`nvcodec`-style pair.
...
- **Caps:** `Caps::CompressedVideo { codec: H264, .. }` Annex-B in, `Caps::RawVideo { format: Nv12, .. }` out as `MemoryDomain::Cuda` (a native `DerivedOutput`). The runtime `CapsChanged` carries the actual cropped display geometry the bitstream declares.
