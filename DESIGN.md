...
2. **The GObject `GstBaseTransform` shell (`libgstglass2glass.so`, the `gstreamer` feature).** A thin C shim (`csrc/gstglass2glass.c`, built by `build.rs` via pkg-config + `cc`) registers `glass2glass` as a real GStreamer element and includes the actual GStreamer headers, so the GObject struct layouts are correct by construction rather than hand-transcribed. It delegates to the C-ABI functions in `src/ffi.rs`, which drive one `BridgeGraph` per instance: `set_caps` builds it from the `fragment` property and the serialized sink caps (normalized: the `(type)` annotations and whitespace GStreamer emits are stripped so g2g's caps reader and launch DSL accept them), `transform_ip` pushes each input buffer and copies back the one processed frame, `stop` destroys it. **v1 is in-place:** the fragment must preserve caps and buffer size (a wgpu effect, `videoflip`, `videoconvert`, an ML preprocessor keeping the pixel format); `BridgeGraph` pins the sub-graph's output caps equal to its input to enforce this and to give a caps-driven transform a fixate target. Caps/size-changing fragments (which need output-buffer allocation and a g2g→`GstCaps` mapping) are future work, as is the zero-copy `GstDmaBufMemory`→`OwnedDmaBuf` import.
