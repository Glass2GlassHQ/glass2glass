...
2. **The GObject `GstBaseTransform` shell (`libgstglass2glass.so`).** A thin C-FFI wrapper that registers `glass2glass` as a real GStreamer element, maps `GstBuffer`/`GstCaps` to/from g2g `Frame`/`Caps` (extracting a `GstDmaBufMemory` fd as a no-op-close `OwnedDmaBuf` on the zero-copy path), and drives one `BridgeGraph` per element instance: `chain` pushes the input buffer and loops `try_pull` to forward whatever the sub-graph has produced. This layer is gated separately because it needs a live `gst-launch` to validate the GObject registration and buffer marshalling.
