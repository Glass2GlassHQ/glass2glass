...
The **zero-copy DMABUF import** path exists at the ingest side: `appsrc` accepts a `MemoryDomain::DmaBuf` frame (`AppSrcFeed::push_dmabuf`), `BridgeGraph::push_dmabuf` feeds it, and the C-ABI `g2g_bridge_push_dmabuf` `dup`s a GStreamer buffer's dma-buf fd (GStreamer keeps the original; g2g's `OwnedDmaBuf` closes only the dup) so no pixel bytes are copied at the boundary. What is *not* yet wired: the fragment's first element must **consume** the `DmaBuf` domain (a GPU import) and the output side must **wrap** a produced `DmaBuf` frame back into a `GstBuffer` (a dma-buf allocator). Until a dma-buf-consuming g2g element lands, a `DmaBuf` frame handed to a system-memory fragment has no consumer, so the shell's default `transform`/`transform_ip` still take the mapped-bytes path; `g2g_bridge_push_dmabuf` is the ready import primitive for when that GPU element exists. That element plus the output wrap-back is the remaining GPU-track work.
