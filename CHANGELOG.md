...
- M1174: `g2g-mcp` tails bus events, and a host application registers its own running pipeline with the in-process `McpServer` while keeping its lifecycle.
- M1173: `g2g-mcp` diagnoses one managed live pipeline through telemetry, logs and packet samples, and applies revision-checked transform changes.
- M1172: `togglerecord name=t` takes gst's `sink_%u` / `src_%u` request pads, expanding into one element per stream in a shared group.
...
- M1159: `fallbackswitch` takes gst's `latency`, widening its stall window and reporting that slack to the runner's latency fold, which now includes fan-in elements.
- M1158: `livesync` holds an early buffer until its due time under `sync`, and keeps the output on one timeline across a restarted input under `single-segment`.
