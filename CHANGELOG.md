...
- **`HlsSrc` now delivers fMP4/CMAF variants**, closing the loop with `Fmp4Demux`
  (M158). The `hls` parser reads `#EXT-X-MAP:URI` (the init segment) into
  `MediaPlaylist::map_uri`. When present, `HlsSrc` fetches the init segment once
  and emits it before the media fragments, so a downstream `fmp4demux` sees the
  `moov` first: `HlsSrc ! fmp4demux ! h264parse ! ...`, the fMP4 analog of the
  existing `HlsSrc ! tsdemux` path.
- **`HlsSrc` negotiates its container by probing the playlist** at negotiation
  (async `intercept_caps` / `caps_constraint`, memoized, like `RtspSrc`'s DESCRIBE):
  it declares `ByteStream{IsoBmff}` when the media playlist has an `#EXT-X-MAP`, else
  `ByteStream{MpegTs}`. The master->media resolution is factored into a shared
  `resolve_media` used by both the probe and `run`.
- Verified end to end by `m156_hlssrc.rs::fmp4_hls_emits_init_then_fragments_and_demuxes`:
  an fMP4 generated by `Mp4Sink` is split into an init segment + media fragments,
  served as an `#EXT-X-MAP` HLS playlist, and `HlsSrc -> Fmp4Demux` recovers the
  original access units. Plus `hls.rs` unit tests for the EXT-X-MAP parse.

### M158: Fmp4Demux (fragmented-MP4 / CMAF byte-stream demux)

- **`Fmp4Demux` demuxes a `ByteStream{IsoBmff}` to `CompressedVideo{H264|H265}`**
  Annex-B access units: the streaming counterpart of the file-based `Mp4Src` and
  the fMP4 analog of `tsdemux`. It buffers and processes one complete top-level box
  at a time (so bytes may arrive in whole segments or split mid-box): the `moov`
  yields codec + geometry (emitted as `CapsChanged`) and parameter sets; each
  `moof`+`mdat` yields samples, with the config-record parameter sets prepended to
  the first access unit. This is what an HLS/DASH fMP4 segment stream feeds.
- **New `Caps::ByteStream{IsoBmff}`** carries the ISO-BMFF / fragmented-MP4 / CMAF
  container; `filesrc` / `httpsrc` accept `bytestream-format=mp4|cmaf|fmp4`.
- **Refactor:** the fMP4 parsing (`parse_header`, `parse_fragments`, `parse_trun`,
  `parse_avcc`/`parse_hvcc`, keyframe + param-set detection) moved from `mp4src`
  into a shared `fmp4` module that both `mp4src` and `fmp4demux` use; no logic
  change (the existing `mp4src` tests still pass).
- Verified by `m158_fmp4demux.rs`: an fMP4 generated by `Mp4Sink` round-trips back
  through `Fmp4Demux` to the exact source access units with the right
  codec/geometry, both as one buffer and fed in 7-byte chunks (box reassembly
