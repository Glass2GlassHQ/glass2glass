...
pub unsafe fn wrap_as_texture(device: &wgpu::Device, shared: SharedNv12Image) -> wgpu::Texture {
    ...
    // and owned here, transferred into the import (freed by the texture's drop
    // callback). A sampled R8Uint plane for `WgpuPreprocess`'s M217 path.
    unsafe {
...
pub unsafe fn wrap_rgba_as_texture(
    ...
    // SAFETY: `device` is the interop Vulkan device; `image` / `memory` are valid
    // and owned here, transferred into the import (freed by the texture's drop
    // callback). A render-target RGBA surface (the wgpu render path draws into it).
    unsafe {
        crate::gpu::import_vk_image_as_wgpu_texture(
            device,
            shared.image,
