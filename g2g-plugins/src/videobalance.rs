...
/// Red/blue byte offsets for a packed 4-channel format (green is always 1, alpha
/// 3). Saturation needs the true R/G/B to weight luma, so BGRA swaps R and B.
fn rb_offsets(format: RawVideoFormat) -> (usize, usize) {
    match format {
        RawVideoFormat::Rgba8 => (0, 2),
        RawVideoFormat::Bgra8 => (2, 0),
        // Negotiation only admits the two packed formats above.
        _ => unreachable!("videobalance: only RGBA/BGRA are negotiated"),
    }
}
...
fn balance_pixel(format: RawVideoFormat, px: &[u8], bright: f64, contrast: f64, sat: f64) -> [u8; 4] {
    let (r_idx, b_idx) = rb_offsets(format);
