...
impl YcbcrToRgba {
    ...
    fn wrap_rgba_texture(&self, image: vk::Image, memory: vk::DeviceMemory) -> wgpu::Texture {
        ...
        // SAFETY: `image` / `memory` were allocated on this wgpu device and are
        // transferred into the import (freed by the texture's drop callback once
        // wgpu is done with the texture).
        unsafe {
...
pub fn readback_rgba_texture(owner: &WgpuRgbaTexture) -> Result<Vec<u8>, G2gError> {
    crate::gpu::read_rgba_texture_dq(owner.device(), owner.queue(), owner.texture())
}
