...
/// True when every cell of a caption grid is blank (a space). Shared by the
...
/// string with its leading and trailing padding trimmed, blank rows dropped.
/// Shared by the CEA-608 `Screen` and CEA-708 `Window` grid renderers.
fn grid_text<R: AsRef<[char]>>(rows: impl IntoIterator<Item = R>) -> String {
    let mut out = String::new();
    for r in rows {
        let line: String = r.as_ref().iter().collect();
...
// ---------------------------------------------------------------------------
// Caption decoder driver (shared by CcExtract and the ST 2110-40 source)
// ---------------------------------------------------------------------------

/// Which in-band caption service to render. A stream commonly carries both legacy
/// CEA-608 line-21 captions and CEA-708 DTVCC at once; this selects the one to
/// surface. The default is CEA-608 CC1 (the primary captions, present in
/// essentially every captioned stream).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CcSource {
    /// A CEA-608 channel (CC1..CC4), each a line-21 field plus in-field channel.
    Cea608(Cea608Channel),
    /// A CEA-708 service number (1 = the primary caption service).
    Cea708(u8),
}

impl Default for CcSource {
    fn default() -> Self {
...
impl CcSource {
    /// Parse a caption-service name (`cc1`..`cc4`, a bare `1`..`4`, `on`/`true` for
    /// CC1, or a CEA-708 `service-N` / `708-N` with N >= 1) into a [`CcSource`].
    /// The vocabulary shared by `uridecodebin`'s `#closed-captions=` and the
    /// ST 2110-40 source's `service` property.
    pub fn parse(v: &str) -> Option<Self> {
        match v.to_ascii_lowercase().as_str() {
            "cc1" | "1" | "on" | "true" => Some(CcSource::Cea608(Cea608Channel::Cc1)),
            "cc2" | "2" => Some(CcSource::Cea608(Cea608Channel::Cc2)),
            "cc3" | "3" => Some(CcSource::Cea608(Cea608Channel::Cc3)),
...
/// The decoder state machine for the selected [`CcSource`]. Both variants are
/// large (the CEA-608 front / back 15x32 screens, the CEA-708 eight-window list),
/// so each is boxed to keep the enum a single pointer.
#[derive(Debug)]
enum Decoder {
    Cea608(Box<Cea608>),
    Cea708(Box<Cea708>),
...
/// Drives a CEA-608 or CEA-708 decoder for one selected service from caption
/// triples: the shared core behind [`crate::ccextract::CcExtract`] (triples mined
/// from H.264 / H.265 SEI) and the ST 2110-40 source (triples carried in a CDP on
/// the wire). Feed it triples with [`push_au`](Self::push_au) (a compressed access
/// unit) or [`push_triples`](Self::push_triples) (already extracted), then drain
/// finished cues with [`take_cues`](Self::take_cues).
#[derive(Debug)]
...
impl CaptionDecoder {
    /// A driver rendering `source`.
    pub fn new(source: CcSource) -> Self {
        let decoder = match source {
            CcSource::Cea608(ch) => Decoder::Cea608(Box::new(Cea608::for_channel(ch))),
            CcSource::Cea708(svc) => Decoder::Cea708(Box::new(Cea708::for_service(svc))),
        };
        Self { source, decoder }
    }

    /// The service this driver renders.
    pub fn source(&self) -> CcSource {
        self.source
    }

    /// Mine one compressed access unit's SEI caption triples and feed them in, each
    /// cue stamped with the access unit's `pts`.
    pub fn push_au(&mut self, au: &[u8], codec: VideoCodec, pts: u64) {
        let triples = extract_cc_data(au, codec);
        self.push_triples(&triples, pts);
    }

    /// Feed already-extracted caption triples (e.g. a -40 CDP's ccdata section) in
    /// transmission order, each cue stamped with `pts`.
    pub fn push_triples(&mut self, triples: &[CcTriple], pts: u64) {
        match &mut self.decoder {
            Decoder::Cea608(dec) => {
                // CEA-608 push_pair takes the channel's field only: CC1/CC2 ride
                // field 1 (`cc_type` 0), CC3/CC4 field 2 (`cc_type` 1).
                let field = match self.source {
                    CcSource::Cea608(ch) => ch.field(),
                    _ => 0,
                };
                for t in triples {
                    if t.cc_type == field {
                        dec.push_pair(t.b0, t.b1, pts);
                    }
                }
            }
            Decoder::Cea708(dec) => {
                // CEA-708 DTVCC rides `cc_type` 2 (packet continuation) / 3 (start).
                for t in triples {
                    dec.push_triple(t.cc_type, t.b0, t.b1, pts);
                }
            }
        }
    }

    /// Drain the cues the decoder has finished.
    pub fn take_cues(&mut self) -> Vec<Cue> {
        match &mut self.decoder {
            Decoder::Cea608(dec) => dec.take_cues(),
            Decoder::Cea708(dec) => dec.take_cues(),
        }
    }

    /// Finalize any still-shown caption at running time `end_ns` (call at EOS).
    pub fn flush(&mut self, end_ns: u64) {
        match &mut self.decoder {
            Decoder::Cea608(dec) => dec.flush(end_ns),
            Decoder::Cea708(dec) => dec.flush(end_ns),
        }
    }
}

// ---------------------------------------------------------------------------
// Caption Distribution Packet (CDP, CEA-708 / SMPTE ST 334-2)
// ---------------------------------------------------------------------------
//
// The payload of a DID 0x61 / SDID 0x01 ancillary packet (ST 2110-40 / SMPTE
// 334-1) is a CDP: a header (0x9669 id, length, frame-rate code, flags, a 16-bit
// sequence counter), then sections (only the ccdata section, id 0x72, carries the
// caption triples), then a footer (0x74, the sequence counter repeated, a
// packet-wide checksum). The ccdata triple byte format is identical to the SEI
// `cc_data` triple (marker | cc_valid | cc_type, then the two data bytes), so this
// just wraps the same triples the SEI path uses.

/// CDP identifier, the first two bytes of every Caption Distribution Packet.
const CDP_IDENTIFIER: u16 = 0x9669;
/// ccdata section id within a CDP.
const CDP_CCDATA_ID: u8 = 0x72;
/// footer id within a CDP.
const CDP_FOOTER_ID: u8 = 0x74;
/// time-code section id (skipped on decode).
const CDP_TIMECODE_ID: u8 = 0x71;

/// Build a Caption Distribution Packet carrying `triples` (up to 31, the 5-bit
/// `cc_count`), the payload of a -40 DID 0x61 caption ANC packet. `frame_rate_code`
/// is the CEA-708 frame-rate nibble (4 = 29.97, 5 = 30, 8 = 60, ...); `seq` is the
/// CDP sequence counter (header and footer must match). The footer's
/// `packet_checksum` makes the byte sum zero mod 256.
pub fn build_cdp(triples: &[CcTriple], frame_rate_code: u8, seq: u16) -> Vec<u8> {
    let cc_count = triples.len().min(0x1F) as u8;
    let mut cdp = Vec::with_capacity(11 + cc_count as usize * 3);
    ...
    for t in triples.iter().take(0x1F) {
        cdp.push(0xF8 | 0x04 | (t.cc_type & 0x03)); // marker | cc_valid | cc_type
        cdp.push(t.b0);
        cdp.push(t.b1);
    }
    // cdp_footer.
...
/// Parse a CDP back into its caption triples, or `None` if it is too short, is not
/// a CDP (bad identifier), its declared length overruns the buffer, or its
/// checksum does not verify. Bounds-checked throughout (never trust the stream):
/// walks the sections from the header, reading the ccdata section and skipping the
/// fixed time-code section, stopping at the footer or any unknown section.
pub fn parse_cdp(data: &[u8]) -> Option<Vec<CcTriple>> {
    // Header (7 bytes) + footer (3) + checksum (1) is the minimum well-formed CDP.
    if data.len() < 11 || u16::from_be_bytes([data[0], data[1]]) != CDP_IDENTIFIER {
        return None;
    }
    let length = usize::from(data[2]);
    // The declared length must fit the datagram; bound the parse to it.
    let data = data.get(..length)?;
    // packet_checksum makes the whole packet sum to zero mod 256.
    if data.iter().fold(0u8, |a, &b| a.wrapping_add(b)) != 0 {
        return None;
    }
    let mut i = 7usize; // past the 7-byte header, at the first section
    let mut out = Vec::new();
    while i < data.len() {
        match data[i] {
            CDP_CCDATA_ID => {
                let count_byte = *data.get(i + 1)?;
                let cc_count = usize::from(count_byte & 0x1F);
                i += 2;
                for _ in 0..cc_count {
                    let triple = data.get(i..i + 3)?;
                    // Only surface valid triples (cc_valid bit set), like the SEI path.
                    if triple[0] & 0x04 != 0 {
                        out.push(CcTriple {
                            cc_type: triple[0] & 0x03,
                            b0: triple[1],
                            b1: triple[2],
                        });
                    }
            ...
            // The time-code section is a fixed 5 bytes (id + 4); skip it.
            CDP_TIMECODE_ID => i += 5,
            // Footer or anything unrecognized ends the sections we read.
            _ => break,
        }
    }
    Some(out)
...
mod tests {
    ...
    fn cdp_skips_invalid_triples_and_a_time_code_section() {
        ...
        cdp.extend_from_slice(&[0xF8 | 0x01, 0x00, 0x00]); // cc_valid clear (cc_type 1) -> dropped
        cdp.extend_from_slice(&[CDP_FOOTER_ID, 0x00, 0x01]);
        cdp[2] = (cdp.len() + 1) as u8;
        let sum = cdp.iter().fold(0u8, |a, &b| a.wrapping_add(b));
    ...
    fn cdp_rejects_bad_identifier_length_and_checksum() {
        ...
        bad_id[0] = 0x00;
        assert!(parse_cdp(&bad_id).is_none(), "wrong identifier rejected");

        let mut over_len = good.clone();
        over_len[2] = 0xFF; // declared length past the buffer
        assert!(parse_cdp(&over_len).is_none(), "over-long length rejected");

        let mut bad_sum = good.clone();
        let last = bad_sum.len() - 1;
        bad_sum[last] ^= 0x01; // corrupt the checksum
        assert!(parse_cdp(&bad_sum).is_none(), "bad checksum rejected");

        assert!(parse_cdp(&[0x96, 0x69]).is_none(), "too short rejected");
    }

    #[test]
    fn caption_decoder_renders_a_608_pop_on_from_triples() {
        // The shared driver: feed the RCL/text/EOC then EDM 608 pairs directly.
