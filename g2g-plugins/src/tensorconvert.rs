...
fn layout_permutation(
    ...
    if from == to {
        let n = numel(in_shape);
        return Some((in_shape.clone(), (0..n).collect()));
    }
    if in_shape.0.len() != 4 {
        return None;
    }
    let d: Vec<usize> = in_shape.0.iter().map(|&x| x as usize).collect();
    match (from, to) {
        (TensorLayout::Nchw, TensorLayout::Nhwc) => {
            let (n, c, h, w) = (d[0], d[1], d[2], d[3]);
            let out = TensorShape(vec![n as u32, h as u32, w as u32, c as u32]);
        ...
        (TensorLayout::Nhwc, TensorLayout::Nchw) => {
            ...
            let out = TensorShape(vec![n as u32, c as u32, h as u32, w as u32]);
            let mut perm = Vec::with_capacity(n * c * h * w);
            for nn in 0..n {
                for cc in 0..c {
                    for hh in 0..h {
...
impl TensorConvert {
    ...
    fn convert_frame(&self, src: &[u8]) -> Result<(Caps, Vec<u8>), G2gError> {
        let (in_dtype, in_shape, in_layout) = self.input.clone().ok_or(G2gError::NotConfigured)?;
...
mod tests {
    ...
    fn layout_transpose_rejected_for_non_4d() {
        let shape = TensorShape(vec![1, 10]);
