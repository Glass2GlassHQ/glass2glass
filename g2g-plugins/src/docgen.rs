...
//! from an element registry, the web counterpart of `g2g-inspect`. Every card
...
//!
//! Run it with a broad feature set so the catalog is complete. The checked-in
//! page adds the hosted Python elements through `g2g-launch-py`:
//!   cargo run -p g2g-python --features launch,g2g-plugins/linux-full
//!     --bin g2g-launch-py -- --docgen docs/elements.html
...
use g2g_core::runtime::{ElementDoc, Registry};

/// Where the page lands when no output path is given.
const DEFAULT_OUT_PATH: &str = "docs/elements.html";
...
/// Write the element reference for `reg` to `out_path` (default
/// `docs/elements.html`), exiting the process if the file cannot be written.
pub fn run(reg: &Registry, out_path: Option<String>) {
    let out_path = out_path.unwrap_or_else(|| DEFAULT_OUT_PATH.to_string());
