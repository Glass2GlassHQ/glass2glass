...
use crate::cea::{extract_cc_data, Cea608, Cea608Channel, Cea708};
...
/// Which in-band caption service [`CcExtract`] renders. A stream commonly carries
/// both legacy CEA-608 line-21 captions and CEA-708 DTVCC at once; this selects
/// the one to surface as text. The default is CEA-608 CC1 (the primary captions,
/// present in essentially every captioned stream).
...
impl CcExtract {
    ...
    /// Feed one access unit's caption triples into the selected decoder, stamping
    /// every cue with the access unit's PTS.
    fn push_au(&mut self, au: &[u8], pts: u64) {
    ...
    /// Finalize any still-shown caption at `end_ns`.
    fn flush_decoder(&mut self, end_ns: u64) {
        match &mut self.decoder {
            ...
            let cues = match packet {
                PipelinePacket::DataFrame(frame) => {
                    ...
                    if let MemoryDomain::System(slice) = &frame.domain {
                        ...
                        self.push_au(&au, frame.timing.pts_ns);
                    }
                    self.take_cues()
                }
                ...
                PipelinePacket::Eos => {
                    self.flush_decoder(self.last_pts);
                    self.take_cues()
                }
            };
            for cue in cues {
