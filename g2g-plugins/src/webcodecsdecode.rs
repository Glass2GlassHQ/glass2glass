    ...
    fn process<'a>(
        ...
        Box::pin(async move {
            ...
            match packet {
                ...
                // (RGBA), not the H.264 input, and we re-announce our own output caps
                // (with the SPS-derived geometry) from `drain_ready`. Input validation
                // lives in `configure_pipeline`, so swallow this like `FfmpegEnc`
                // rather than rejecting it. (A mid-stream codec swap is a follow-up;
                // it would arrive on the input link, not here.)
...
/// Copy one `VideoFrame` out as packed RGBA: `(bytes, width, height, pts_ns)`.
/// `copyTo` is asked to convert to RGBA; the copy targets a JS `Uint8Array` (not
...
/// The decoded frame's VISIBLE pixel dimensions: the region `copyTo` (and an
/// external-texture sample) covers, which the RGBA buffer is sized to. NOT
/// `display_width`/`display_height`, which apply the stream's sample-aspect-ratio
/// (SAR) and diverge from the pixel buffer for non-square pixels (e.g. an
/// anamorphic or stretched source), announcing a geometry the copied bytes don't
/// match (a `CapsMismatch` at the sink). Also correct for the coded-padding case
/// (coded 640x482 for a 640x480 visible rect). Falls back to coded dims if the
/// visible rect is unavailable.
fn visible_dims(frame: &VideoFrame) -> (u32, u32) {
    match frame.visible_rect() {
