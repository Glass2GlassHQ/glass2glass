...
//! (`Caps::CompressedVideo{H264}` AVCC, or `Caps::Audio{Aac}`), an FLV byte stream
//! out (`Caps::ByteStream{Flv}`).
//!
...
//! the input caps at configure. CPU, `no_std` baseline.
//!
//! ```text
        ...
        Box::pin(async move {
            ...
            match packet {
                PipelinePacket::DataFrame(frame) => {
                    ...
                    let flv = mux.push_au(slice.as_slice(), pts_ms);
...
mod tests {
    ...
    async fn element_round_trips_through_flvdemux() {
        let au0 = alloc::vec![0x65u8, 0xAA, 0xBB];
        let au1 = alloc::vec![0x41u8, 0xCC];
