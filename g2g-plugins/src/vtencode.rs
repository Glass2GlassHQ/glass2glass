//! M231: macOS hardware H.264 encode via VideoToolbox (`VTCompressionSession`).
//!
...
//! `DataFrame`s (`MemoryDomain::System`) and produces H.264 access units, also
//! `MemoryDomain::System`, in Annex-B framing (what the rest of the pipeline
//! expects).
//!
...
use crate::annexb::avcc_to_annexb;
...
impl AsyncElement for VtEncode {
    ...
    fn configure_pipeline(&mut self, absolute_caps: &Caps) -> Result<ConfigureOutcome, G2gError> {
        ...
        let state = unsafe { build_session(w, h, self.bitrate, self.keyframe_interval)? };
    ...
    fn metadata(&self) -> g2g_core::ElementMetadata {
        g2g_core::ElementMetadata::new(
            ...
            "Codec/Encoder/Video",
            "Hardware H.264 encode on macOS via VideoToolbox (NV12 -> Annex-B)",
            "g2g",
        )
    }
...
impl PadTemplates for VtEncode {
    fn pad_templates() -> Vec<PadTemplate> {
        ...
        Vec::from([PadTemplate::sink(CapsSet::one(nv12)), PadTemplate::source(CapsSet::one(h264))])
    }
}
...
unsafe extern "C-unwind" fn output_callback(
    ...
    match unsafe { sample_to_annexb(sample) } {
...
unsafe fn sample_to_annexb(sample: &CMSampleBuffer) -> Result<(Box<[u8]>, u64, bool), G2gError> {
    ...
    let keyframe = h264_au_is_keyframe(&annexb);
    if keyframe {
        // Prepend the parameter sets (Annex-B framed) ahead of the IDR.
        if let Some(fmt) = unsafe { CMSampleBufferGetFormatDescription(sample) } {
            let mut prefix = unsafe { parameter_sets_annexb(&fmt)? };
...
/// Extract the H.264 SPS/PPS from a `CMVideoFormatDescription`, each Annex-B
/// framed (4-byte start code), in order.
///
/// SAFETY: `fmt` is a valid H.264 `CMVideoFormatDescription`.
unsafe fn parameter_sets_annexb(fmt: &CMFormatDescription) -> Result<Vec<u8>, G2gError> {
    let hw = || G2gError::Hardware(HardwareError::Other);
    let mut out = Vec::new();
