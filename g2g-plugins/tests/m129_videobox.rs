//! M129 `videobox`: the border element is registered in `default_registry` and
//! runs in a text pipeline (changing geometry), with its `Uint` / `Str`
...
async fn videobox_runs_in_a_text_pipeline() {
    ...
    let graph = parse_launch(
        &reg,
        "videotestsrc num-buffers=3 pattern=smpte ! videobox border-left=8 border-right=8 fill=black ! fakesink",
    )
