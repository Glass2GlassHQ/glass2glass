...
async fn sink_retries_connect_until_server_is_up() {
    ...
    let sender = async move {
        ...
        sink.configure_pipeline(&test_caps()).expect("configure defers connect");
    ...
    let receiver = async move {
        ...
        let mut src = RemoteSrc::from_listener(listener).unwrap().with_frame_limit(N);
    ...
    assert_eq!(emitted, N, "all frames crossed after the sink retried the connect");
...
async fn source_reaccepts_after_sender_drops() {
    ...
    let dest = format!("127.0.0.1:{port}").parse().unwrap();
    let mut src = RemoteSrc::from_listener(listener).unwrap().with_reconnect().with_frame_limit(TOTAL);
    ...
    let sender = async move {
        ...
        // First sender: caps + 2 frames, then drop (no Eos) by going out of scope.
        {
            ...
            for i in 0..2u64 {
                s1.process(PipelinePacket::DataFrame(frame(i)), &mut null).await.expect("s1 frame");
            }
        ...
        for i in 2..TOTAL {
            let _ = s2.process(PipelinePacket::DataFrame(frame(i)), &mut null).await;
        }
    ...
    let recv = tokio::time::timeout(
        ...
        run_simple_pipeline(&mut src, &mut collect, &clock, LatencyProfile::Live.link_capacity()),
    );
    let (recv_res, ()) = tokio::join!(recv, sender);
    let stats = recv_res.expect("finishes within 10s").expect("receive ok");

    assert_eq!(stats.frames_emitted, TOTAL, "all frames across both senders delivered");
    assert_eq!(collect.seqs, (0..TOTAL).collect::<Vec<_>>(), "frames in order across the reconnect");
    // The initial caps (configure) plus the replacement's re-sent caps.
    assert!(collect.caps.len() >= 2, "re-accepted client re-sent its caps: {:?}", collect.caps);
