...
async fn cuda_shares_vulkan_external_memory() {
    ...
    assert!(
        shared.size as usize >= (320 * 360) as usize,
        "image spans at least the packed NV12"
    );
    ...
    assert!(
        matched,
        "the NV12 pattern read back through the CUDA array must match"
    );
...
async fn wgpu_reads_what_cuda_wrote() {
    ...
    eprintln!(
        "wgpu read back {}x{} texels, {mismatches} mismatches",
        w, tex_h
    );
    assert_eq!(
        mismatches, 0,
