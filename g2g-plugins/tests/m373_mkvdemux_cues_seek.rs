...
async fn mkvdemux_cues_seek_jumps_straight_to_the_target_cluster() {
    ...
    assert_eq!(requested.start, cluster1_offset, "Cues seek targets Cluster1, not a re-scan from 0");
    assert_eq!(&file[cluster1_offset as usize..cluster1_offset as usize + 4], &[0x1F, 0x43, 0xB6, 0x75]);
    ...
    demux.process(data_frame(&cluster1_bytes), &mut post).await.unwrap();
