...
//! an fMP4 byte stream. The fixture is generated by `Mp4Sink` (the inverse), so
...
/// Mux the access units to an fMP4 byte buffer via `Mp4Sink`.
async fn make_fmp4(aus: &[Vec<u8>]) -> Vec<u8> {
    let path = temp_path("src");
    let mut sink = Mp4Sink::new(&path);
    sink.configure_pipeline(&h264_caps(64, 48)).unwrap();
    let mut out = NullOut;
    for (i, au) in aus.iter().enumerate() {
        sink.process(PipelinePacket::DataFrame(frame(au.clone(), i as u64 * 33_333_333, i as u64)), &mut out)
            .await
            .unwrap();
    }
...
#[tokio::test]
async fn demuxes_fmp4_from_mp4sink_roundtrip() {
