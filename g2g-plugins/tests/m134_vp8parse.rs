...
fn vp8_keyframe(width: u32, height: u32) -> Vec<u8> {
    vec![
        0x00,
        0x00,
        0x00, // frame tag: key frame, version 0
        0x9d,
        0x01,
        0x2a, // start code
        (width & 0xFF) as u8,
...
async fn mkvdemux_feeds_vp8parse_end_to_end() {
    ...
    let src = graph.add_source(GraphNodeRef::Source(Box::new(MkvSource {
        bytes: Some(synthetic_vp8_webm()),
    })));
    let demux = graph.add_transform(GraphNodeRef::element(
        MkvDemux::new().with_stream(MkvStream::Vp8),
    ));
...
fn vp8parse_registered_and_constructable() {
    ...
    assert!(
        reg.inspect("vp8parse").is_some(),
        "vp8parse joins the default registry"
    );
    assert!(
        reg.make_element("vp8parse").is_some(),
        "vp8parse builds by name"
    );
}
