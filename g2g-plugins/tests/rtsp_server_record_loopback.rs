...
async fn rtsp_publisher_handshakes_then_pushes_rtp() {
    ...
    // Server source on an ephemeral RTSP port.
    let listener = StdTcpListener::bind("127.0.0.1:0").expect("bind rtsp");
    let rtsp_addr = listener.local_addr().unwrap();

    // The publisher: connect, ANNOUNCE/SETUP/RECORD, then packetize + push RTP.
    let publisher = async move {
        ...
        // Push N access units as RTP to the server's negotiated port.
        ...
        // Keep the control connection open until the server has drained.
        tokio::time::sleep(std::time::Duration::from_millis(200)).await;
        drop(ctrl);
    };

    // The server source: accept the publisher, handshake to RECORD, depayload N
    ...
    let (_, (n, tags)) = tokio::join!(publisher, server);
