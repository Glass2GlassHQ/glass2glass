...
use g2g_core::{
    ...
    PadTemplate, PadTemplates, Rate, RawVideoFormat, VideoCodec,
};
...
fn temp_uri(tag: &str, bytes: &[u8]) -> (std::path::PathBuf, String) {
    let path = std::env::temp_dir().join(format!("g2g_m382_{}_{}.bin", std::process::id(), tag));
...
fn frame(data: Vec<u8>, pts_ns: u64) -> PipelinePacket {
    PipelinePacket::DataFrame(g2g_core::frame::Frame::new(
        ...
        g2g_core::frame::FrameTiming {
            pts_ns,
            dts_ns: pts_ns,
            ..g2g_core::frame::FrameTiming::default()
        },
        0,
    ))
}
...
async fn playbin_uri_auto_fans_out_a_matroska_file() {
    ...
    // FileSrc -> MkvDemuxN(2). Video port: demux -> decoder -> auto deinterlace
    // (M935) -> auto video sink. Audio port: demux -> decoder -> audioconvert ->
    // audioresample -> auto audio sink (the M422+ audio branch, so the sink sees
    // a fixed PCM format while the converters absorb the stream's real channels
    // / rate).
    // Nodes: source + demux + video(decoder+deinterlace+sink)
    //   + audio(decoder+convert+resample+sink) = 9.
    // Edges: src->demux(1) + video branch(3) + audio branch(4) = 8.
    assert_eq!(
        graph.node_count(),
        9,
    ...
    assert_eq!(
        graph.edges().len(),
        8,
        "video branch (3 edges) + audio branch (4 edges) + src->demux"
    );
}
...
async fn playbin_uri_non_matroska_falls_through() {
    ...
    assert_eq!(
        graph.node_count(),
        3,
        "non-Matroska declines: single-stream playbin"
    );
}
...
fn non_lone_playbin_is_not_hooked() {
    ...
    let mut reg = base_registry();
    reg.register_launch(LaunchFactory::of::<NullSink>("fakesink", || {
        Box::new(NullSink)
    }));
