...
fn decodes_idr_frame_to_luma() {
    ...
    // The decode ran on real GPU hardware and produced a correct picture: persist
    // `Hardware` conformance evidence tagged with the GPU it validated on, so
    // `g2g-inspect --maturity` derives `vulkanvideo` (H.264) as HardwareValidated.
    ...
    g2g_plugins::conformance::persist::record_evidence(
        ...
        &Evidence::new(ConformanceDimension::Hardware)
            ...
            .codec("h264")
            .detail("Vulkan Video IDR decode to non-uniform NV12 luma"),
    )
