...
fn videotestsrc_props_round_trip() {
    ...
    src.set_property("pattern", PropValue::Str("snow".into()))
        .unwrap();
    ...
    src.set_property("framerate", PropValue::Fraction(25, 1))
        .unwrap();

    assert_eq!(
        src.get_property("pattern"),
        Some(PropValue::Str("snow".into()))
    );
...
fn property_errors_are_typed() {
    ...
    // Unknown name.
    assert_eq!(
        src.set_property("nope", PropValue::Int(1)),
        Err(PropError::Unknown)
    );
    // Right name, wrong value kind (width wants a uint).
    assert_eq!(
        src.set_property("width", PropValue::Str("x".into())),
        Err(PropError::Type)
    );
    ...
    assert_eq!(
        src.set_property("framerate", PropValue::Fraction(30, 0)),
        Err(PropError::Value)
    );
}
...
fn spec_tables_describe_each_property() {
    ...
    let names: Vec<_> = SourceLoop::properties(&src)
        .iter()
        .map(|s| s.name)
        .collect();
    ...
    let fr = SourceLoop::properties(&src)
        .iter()
...
fn videorate_fraction_property() {
    ...
    rate.set_property("framerate", PropValue::Fraction(10, 1))
        .unwrap();
    assert_eq!(
        rate.get_property("framerate"),
        Some(PropValue::Fraction(10, 1))
    );
}
...
fn set_property_through_dyn_erasure() {
    ...
    src.set_property("pattern", PropValue::Str("moving-bar".into()))
        .unwrap();
    assert_eq!(
        src.get_property("pattern"),
        Some(PropValue::Str("bar".into()))
    );
    ...
    assert_eq!(
        flip.get_property("method"),
        Some(PropValue::Str("vertical-flip".into()))
    );
}
