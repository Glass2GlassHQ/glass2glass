//! Distributed-graph primitive over a WebSocket boundary (M554): the WebSocket
//! sibling of `remote_loopback.rs`. The near side drives a `RemoteWsSink`
//! (WebSocket client, serializing the `PipelinePacket` stream via the g2g-core
//! wire codec, one packet per binary message); the far side runs a `RemoteWsSrc`
//! (WebSocket server) -> collecting sink pipeline that reconstructs the identical
//! stream. Proves the leading `CapsChanged` is discovered from the wire, every
//! `DataFrame`'s bytes / timing / sequence survive the WebSocket round trip, and
//! the stream ends on `Eos`. The wire codec's per-variant fidelity is unit-tested
//! in `g2g_core::wire`; this asserts the WebSocket message framing carries it.
            ...
            match packet {
                PipelinePacket::DataFrame(frame) => {
                    if let MemoryDomain::System(slice) = &frame.domain {
                        self.frames.push((frame.sequence, frame.timing, slice.as_slice().to_vec()));
                    }
                }
...
#[tokio::test]
async fn remote_ws_transport_carries_a_split_graph_edge() {
    const N: u8 = 8;
    ...
    let mut src = RemoteWsSrc::from_listener(listener).unwrap().with_frame_limit(N as u64);
    let mut sink = CollectSink::default();
    let clock = ZeroClock;

    // Near side: drive a RemoteWsSink dialing the far-side port over ws://,
    // sending the negotiated caps then N tagged frames then Eos. The WebSocket
    // handshake is async, so (unlike the TCP sink) the connect happens inside the
    // first `process`; configure_pipeline only records the caps.
    let sender = async {
        let url = format!("ws://127.0.0.1:{port}");
        let mut remote = RemoteWsSink::new(url);
        ...
        for i in 0u8..N {
            ...
            if remote.process(PipelinePacket::DataFrame(frame), &mut null).await.is_err() {
                break;
            }
        }
    ...
    let stats = recv_res.expect("receiver finishes within 10s").expect("receive pipeline ok");

    // Every frame delivered.
    assert_eq!(stats.frames_emitted, N as u64, "all frames crossed the boundary");
    assert!(sent >= (N as u64 + 1), "sender emitted caps + {N} frames: {sent}");

    // The far side discovered the sender's caps from the wire.
    assert!(!sink.caps.is_empty(), "caps were carried");
    assert_eq!(sink.caps[0], test_caps(), "discovered caps match the sender's");
