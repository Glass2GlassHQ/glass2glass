...
async fn mux_two_clusters() -> Vec<u8> {
    ...
    mux.process(data_frame(&[0x01], 0), &mut sink).await.unwrap();
    mux.process(data_frame(&[0x02], 40_000_000), &mut sink).await.unwrap();
    mux.process(data_frame(&[0x04], 1_200_000_000), &mut sink).await.unwrap();
    mux.process(data_frame(&[0x05], 1_240_000_000), &mut sink).await.unwrap();
...
fn nth_cluster_offset(file: &[u8], n: usize) -> usize {
    file.windows(4).enumerate().filter(|(_, w)| *w == CLUSTER_ID).map(|(i, _)| i).nth(n).unwrap()
}
...
async fn mkvmux_cues_make_its_own_output_seekable() {
    ...
    assert!(file.windows(4).any(|w| w == [0x1C, 0x53, 0xBB, 0x6B]), "a Cues element was written");
    ...
    assert_eq!(pre.frames, vec![vec![0x01u8], vec![0x02], vec![0x04], vec![0x05]]);
    ...
    let requested = byte.take_pending().expect("an upstream byte-seek was requested");
    assert_eq!(
    ...
    let cues_offset = file.windows(4).position(|w| w == [0x1C, 0x53, 0xBB, 0x6B]).unwrap();
    let cluster1_bytes = &file[cluster1_offset as usize..cues_offset];
    demux.process(data_frame(cluster1_bytes, 0), &mut post).await.unwrap();
    ...
    assert_eq!(post.frames, vec![vec![0x04u8], vec![0x05]], "re-synced from Cluster 2 via the Cues");
}
