...
impl AsyncElement for OrderedRecordingSink {
    ...
    fn process<'a>(
        ...
        let event = match &packet {
            ...
            // Future control packets: not recorded, like Segment.
    ...
    fn reconfigure(&mut self, request: Reconfigure) -> Result<Caps, G2gError> {
        *self.reconfigure_calls.lock().unwrap() += 1;
        match request {
            ...
            Reconfigure::Renegotiate
            | Reconfigure::ForceKeyframe
            | Reconfigure::AbsorbOrientation => Err(G2gError::FixationFailed),
        }
    }
