...
fn muxed_ts_variant_overlays_in_sei_captions() {
    ...
    let graph = build_hls_ts_cc_overlay(
        &reg,
        "https://h/master.m3u8",
        &streams,
    ...
    assert_eq!(
        fan_outs, 2,
        "the demux (video+audio) and the caption tee both fan out by two"
    );
    ...
    assert_eq!(
        *fan_ins[0], 2,
        "the overlay joins the decoded video and the extracted captions"
    );
...
fn separate_audio_variant_overlays_captions_across_sources() {
    ...
    assert_eq!(
        source_count(&graph),
        2,
        "video master + separate audio rendition"
    );
