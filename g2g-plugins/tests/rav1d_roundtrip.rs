#![cfg(all(feature = "rav1d", feature = "av1-encode"))]
//! Pure-Rust AV1 decode round-trip: encode I420 frames to AV1 with the pure-Rust
//! `Av1Enc` (rav1e), then decode them back with `Rav1dDec` (`re_rav1d`, the Rust
//! port of dav1d) and check the recovered frames are correctly-sized I420 of the
//! encoded geometry whose luma is the flat grey that went in (AV1 is lossy, so
//! within a tolerance). Proves the rav1d decode path end to end without an
//! external fixture or any system dependency (no libdav1d, no NASM).
...
#[tokio::test]
async fn av1_encode_then_rav1d_decode_round_trips_i420() {
    ...
    // Decode the AV1 packets back with the pure-Rust re_rav1d decoder.
    let mut dec = Rav1dDec::new();
    ...
    assert!(!decoded.frames.is_empty(), "rav1d decoded at least one frame");
...
/// whose luma survives the lossy round trip. Proves the pure-Rust multi-plane /
/// subsampling packing path beyond 4:2:0.
async fn roundtrip_chroma(format: RawVideoFormat) {
    ...
    assert!(
        ...
        "rav1d announced {format:?} 64x64, got {:?}",
        decoded.caps,
    );
    assert!(!decoded.frames.is_empty(), "rav1d decoded at least one {format:?} frame");
...
#[tokio::test]
async fn av1_4_2_2_round_trips_through_rav1d() {
    roundtrip_chroma(RawVideoFormat::I422).await;
}

#[tokio::test]
async fn av1_4_4_4_round_trips_through_rav1d() {
...
/// with `Rav1dDec`, asserting the decoder announces the high-bit-depth format and
...
#[tokio::test]
async fn av1_10bit_4_2_0_round_trips_through_rav1d() {
    roundtrip_high_depth(RawVideoFormat::I420p10).await;
}

#[tokio::test]
async fn av1_12bit_4_2_0_round_trips_through_rav1d() {
    roundtrip_high_depth(RawVideoFormat::I420p12).await;
}

#[tokio::test]
async fn av1_10bit_4_2_2_round_trips_through_rav1d() {
    roundtrip_high_depth(RawVideoFormat::I422p10).await;
}

#[tokio::test]
async fn av1_12bit_4_4_4_round_trips_through_rav1d() {
