...
fn properties_drive_the_output_caps() {
    ...
    src.set_property("format", PropValue::Str("NV12".into())).unwrap();
    src.set_property("num-buffers", PropValue::Int(5)).unwrap();
    assert_eq!(src.get_property("width"), Some(PropValue::Uint(640)));
    assert_eq!(src.get_property("format"), Some(PropValue::Str("NV12".into())));
...
fn pysrc_parses_as_a_launch_source() {
    ...
    assert!(graph.is_ok(), "pysrc should parse as a launch source: {:?}", graph.err());
}
...
fn produces_frames_until_python_signals_eos() {
    ...
    use core::pin::Pin;
    use g2g_core::{G2gError, Frame, MemoryDomain, OutputSink, PipelinePacket, PushOutcome};
