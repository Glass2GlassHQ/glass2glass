...
fn cmd_wasm() -> i32 {
    ...
    // web-gpu (superset of web-codecs): the WebGPU zero-copy presentation surface.
    let code = run(
    ...
    // The deployable browser cdylib. Excluded from the workspace, so drive it by
    // manifest path; same unstable cfg (it pulls g2g-plugins' web-codecs). This is
    // a plain check (no wasm-pack needed) so it can run in CI.
    run(
        "wasm check (g2g-web cdylib)",
        "cargo",
        &[
            "check",
            "--manifest-path",
