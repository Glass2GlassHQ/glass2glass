...
impl<const N: usize, const BYTES: usize> StaticLendRing<N, BYTES> {
    // Associated consts as array-repeat operands: the MSRV-1.75 way to build
    // the arrays in a `const fn` (inline-const repeat needs 1.79). The lint
    // warns because writing through a const copies, but copying fresh values
    // into the array is exactly the intent; nothing ever reads these consts.
    #[allow(clippy::declare_interior_mutable_const)]
    const EMPTY_SLOT: UnsafeCell<[u8; BYTES]> = UnsafeCell::new([0u8; BYTES]);
    #[allow(clippy::declare_interior_mutable_const)]
    const UNLEASED: AtomicBool = AtomicBool::new(false);
    ...
    pub fn acquire(&self) -> Option<RingSlot<'_, N, BYTES>> {
        // The slot handle carries the slot / lease *references* (resolved here,
        // where `idx` is loop-bounded) rather than re-indexing with a stored
        // index at each use site: the no-alloc build proves itself free of
        // bounds-check panic paths, so no use site may depend on the optimizer
        // rediscovering the `idx < N` invariant.
        for idx in 0..N {
...
impl<const N: usize, const BYTES: usize> RingSlot<'_, N, BYTES> {
    ...
    pub fn buf_mut(&mut self) -> &mut [u8] {
        // SAFETY: this `RingSlot` is the unique holder of its slot's lease and the
        // slot is not yet published, so this is the only reference to those bytes.
        let arr: &mut [u8; BYTES] = unsafe { &mut *self.slot.get() };
        arr.as_mut_slice()
    }
    ...
    pub unsafe fn publish(self, len: usize) -> SystemSlice {
        ...
        let ptr = self.slot.get() as *const u8;
        let flag = self.lease as *const AtomicBool as *mut c_void;
