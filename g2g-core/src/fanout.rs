...
/// a `u32`, so `u64::MAX` can never collide).
const NO_BITRATE: u64 = u64::MAX;

/// A per-input reverse-signal handle a fan-in session shares with the runner, so
...
/// `Rtc` / network handle) writes with [`request_keyframe`](Self::request_keyframe)
/// / [`set_bitrate`](Self::set_bitrate); the runner's per-source adapter reads
/// the highest-priority pending signal with [`take`](Self::take) after each push
/// and surfaces it to that source as a [`PushOutcome`], exactly as a linked sink
/// would. Reconfigure outranks bitrate, matching the linear reverse channel.
#[derive(Debug, Clone)]
...
impl Default for ReverseChannel {
    fn default() -> Self {
        Self {
            keyframe: Arc::new(AtomicBool::new(false)),
            bitrate: Arc::new(AtomicU64::new(NO_BITRATE)),
        }
    }
}
...
impl ReverseChannel {
    ...
    /// Session side: request an upstream keyframe (a remote PLI for this track).
    pub fn request_keyframe(&self) {
        self.keyframe.store(true, Ordering::Relaxed);
    }

    /// Session side: relay a target send bitrate (bits/second) for this track.
    pub fn set_bitrate(&self, bps: u32) {
        self.bitrate.store(bps as u64, Ordering::Relaxed);
    }

    /// Runner side: consume the highest-priority pending signal as a
    /// [`PushOutcome`], or `None` if nothing is pending. Reconfigure (keyframe)
    /// takes priority over a bitrate update, as on the linear reverse channel.
    pub fn take(&self) -> Option<PushOutcome> {
        if self.keyframe.swap(false, Ordering::Relaxed) {
            return Some(PushOutcome::Reconfigure(Reconfigure::ForceKeyframe));
        }
        match self.bitrate.swap(NO_BITRATE, Ordering::Relaxed) {
            NO_BITRATE => None,
            bps => Some(PushOutcome::Bitrate(bps as u32)),
        }
    }
}
...
pub trait MultiDuplexSession: ElementBound {
    ...
    /// A [`ReverseChannel`] for send-side input pad `input`, if this session routes
    /// reverse signals (WebRTC PLI / BWE arriving on that track's m-line) back to
    /// the individual upstream source feeding it. The duplex runner clones it
    /// before running and polls it after each push from that source, surfacing any
    /// pending signal as a [`PushOutcome`], exactly as
    /// [`MultiInputElement::reverse_channel`] does for a fan-in session. Default
    /// `None`: no per-input reverse channel.
...
pub trait MultiInputElement: ElementBound {
    ...
    /// A [`ReverseChannel`] for input pad `input`, if this session routes reverse
    ...
    /// per-input reverse channel (a plain muxer imposes none).
    fn reverse_channel(&self, _input: usize) -> Option<ReverseChannel> {
        None
    }
