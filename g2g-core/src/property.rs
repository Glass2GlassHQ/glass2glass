...
/// Static metadata for one settable property: its name, type, and a one-line
/// description. The element type declares these (via
/// [`properties`](crate::AsyncElement::properties)) so tooling can enumerate and
/// document them without a live instance carrying the strings.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Convenience: format a property spec table the way `gst-inspect` lists it, one
/// line per property. Used by the registry's introspection dump (M105).
...
mod tests {
    ...
    #[test]
    fn format_specs_lists_one_line_each() {
        let specs = [
        ...
        assert!(dump.contains("pattern: string - test pattern"));
        assert!(dump.contains("num-buffers: int -"));
